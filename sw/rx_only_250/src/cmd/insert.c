...
#include "driver/include/rx_vitis_net_p4_core_defs.h"
...
int main(int argc, char *argv[]) {
    ...
    result = XilVitisNetP4TargetInit(&targetCtx, &interface, &XilVitisNetP4TargetConfig_rx_vitis_net_p4_core);
    ...
    logInfo("Inserting MacAddrs Table Entry");
    uint8_t key[6] = SAMPLE_KEY;
    uint8_t value[6] = SAMPLE_VALUE;
    result = XilVitisNetP4TableInsert(macAddrsTable.tableCtx, key, NULL, 0, macAddrsTable.replaceSrcActionId, value);
    if (result != XIL_VITIS_NET_P4_SUCCESS) {
        logError("Code: %s", XilVitisNetP4ReturnTypeToString(result));
        cleanupTarget(targetCtx);
        cleanupPciResource(pciResource);
